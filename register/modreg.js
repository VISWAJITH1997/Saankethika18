        ...
        this.$get = ["$$rAF", "$rootScope", "$rootElement", "$document", "$$HashMap", "$$animation", "$$AnimateRunner", "$templateRequest", "$$jqLite", "$$forceReflow", function (b, c, g, l, C, Wa, Q, t, H, T) {
            ...
            function p(a, h, v) {
                ...
                if (H) {
                    ...
                    else return W(a, v, w), v.runner;
                    else if (d("join", a, w, v))
                        if (2 === v.state) W(a, w, {});
                        else return Sa(a, g ? h : null, m), h = w.event = v.event, m = W(a, v, w), v.runner
                } else W(a, w, {});
        ...
        this.$get = ["$window", "$$jqLite", "$$AnimateRunner", "$timeout", "$$forceReflow", "$sniffer", "$$rAFScheduler", "$$animateQueue", function (a, e, l, n, I, g, u, C) {
            ...
            return function (a, c) {
                ...
                var x = [],
                    I = a.attr("class"),
                    E = Na(f),
                    w, O, F, G, v, J, R, m, V, W, fa = [];
                if (0 === f.duration || !g.animations && !g.transitions) return L();
                var ja = f.event && Y(f.event) ? f.event.join(" ") : f.event,
                    $ = "",
                    U = "";
...
angular.module("invente", ['ngAnimate'])
    .controller("modRegFormController", ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
        ...
        $scope.modreg = function () {
            ...
            $http.post("onlinemodify.php", $scope.data, {}).then(
                function (res) {
                    console.log(res);
                    $scope.data = {
                        key: "",
                    ...
                    if (res.data.success === true) {
                        $scope.recommended = true;
                        $scope.errMsg = "Successfully Modified";
                        $timeout(function () {
                            $scope.popup = false;
                        }, 6000);
                    } else {
